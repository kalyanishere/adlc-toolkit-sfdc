...
- `sessionId`: **string | null**, captured once in Phase 0 from `$CLAUDE_CODE_SESSION_ID` (with fallback to legacy `$CLAUDE_SESSION_ID`) and never overwritten thereafter. The sprint dashboard joins on this value to attribute Claude Code transcript token usage (`~/.claude/projects/<flattened-cwd>/*.jsonl`) back to this REQ for per-phase token rollups. On resume, preserve the existing value — overwriting orphans token history from the original session.
