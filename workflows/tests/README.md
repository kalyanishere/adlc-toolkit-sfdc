...
`adlc-sprint` Dynamic Workflows engine — its consolidation logic, max-N bound,
and cross-REQ merge grouping. The Workflow runtime has no `require`/`import`/`fs`,
so the engine is ONE self-contained file
([`adlc-sprint.workflow.js`](../adlc-sprint.workflow.js)) and those helpers are
**inlined** behind the `// ==== BEGIN/END PURE ====` sentinels. The shared loader
[`_load-pure.js`](./_load-pure.js) reads just that section and evaluates it with
the runtime globals absent, so `node:test` can cover the logic with
**no build step**.
...
silently regress (the BR-7 review-consolidation gate, the BR-12 max-5 bound,
cross-REQ merge grouping), so they get real, deterministic unit coverage here.
...
| `reflectorQuestions`, `fixedPairs`, `mergeReverified` | Halt/merge-driving Phase-5 helpers. |
| `allMerged`, `stripVerifyMarkers`, `sharesRepo`, `terminalValue` | Phase-8 ground-truth and shared schema-shape helpers. |
...
Expected: all tests pass (`ℹ pass 33  ℹ fail 0`).
