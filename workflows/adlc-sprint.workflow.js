...
function preflightPrompt(reqs) {
  return [
    ...
    '  - id: the REQ id.',
    '  - eligible: true UNLESS the REQ is already fully merged. The bar is',
    '    intentionally low — the workflow runs the full lifecycle:',
    '      Phase 1 validates the spec (== /validate against requirement.md);',
    '      Phase 2 runs the explore trio + architect, which CREATES the tasks/',
    '      directory; Phase 3 validates that architecture+tasks output.',
    '    Therefore eligibility MUST NOT require a pre-existing tasks/',
    '    directory and MUST NOT require requirement.md to be "approved" — a',
    '    "draft" spec passes here and is gated at Phase 1 instead. The only',
    '    eligibility blockers are: (a) requirement.md missing entirely;',
    '    (b) the REQ is already fully merged on the integration branch;',
    '    (c) a worktree path collision with a different in-flight branch',
    '    (so /proceed Phase 0 would fail loud). Anything else — draft spec,',
    '    no tasks/, no architecture.md — is what Phases 1-3 are for.',
