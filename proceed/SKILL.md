...
```json
{
  ...
  "sessionId": "cdfa84aa-57d1-4e64-999d-ce37c548c13a",
  "completed": false,
  "currentPhase": 0,
...
5a. **Initialize `pipeline-state.json` in the primary repo's MAIN CHECKOUT** at `<primary-repo-path>/.adlc/specs/REQ-xxx-*/pipeline-state.json` with `currentPhase: 0, currentPhaseStartedAt: <now>, completedPhases: [], completed: false, startedAt: <now>, sessionId: <CLAUDE_SESSION_ID>, integrationBranch: <integration-branch>, repos: {...resolved registry, paths/branches set, worktree: null for each repo since worktrees do not yet exist...}, mergeOrder: [...from config.yml or declared order, filtered to touched repos...], phase4: { currentTask: null, completedTasks: [], failedTasks: [] }`. **Pre-worktree state file**: this initial write goes to the main checkout, NOT a worktree, because worktrees are created later (Step 1.5). The sprint dashboard's worktree-aware scan picks up this main-checkout file within ~1.5s, so Phase 0 and Phase 1 are visible the moment `/proceed` starts. Step 1.5 will move this file into the worktree once one exists.

   **Capture `sessionId` once, here, never overwrite.** Resolve the value via Bash: `echo "${CLAUDE_CODE_SESSION_ID:-${CLAUDE_SESSION_ID:-}}"` (Claude Code 2.x exports `CLAUDE_CODE_SESSION_ID`; older builds used the unprefixed `CLAUDE_SESSION_ID` — the fallback handles both). The sprint dashboard's token-usage aggregator reads `~/.claude/projects/<flattened-cwd>/*.jsonl`, filters messages by this `sessionId`, and rolls them up to per-REQ + per-project token counts (input / output / cache write / cache read) bucketed by phase. Without `sessionId` the dashboard renders "tokens n/a" for the REQ. If neither variable is set (a hook environment that doesn't export either), set `sessionId: null` — the dashboard handles missing data gracefully. On resume, preserve the existing `sessionId` (it links the pipeline to its original session's transcripts; overwriting would orphan token history).
